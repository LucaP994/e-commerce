...
public class SecurityConfig {
    ...
    Collection generateAuthoritiesFromClaim(Collection roles) {
        List r = (List) roles.stream().collect(Collectors.toList());
        for(int i = 0; i < r.size(); i++){
            System.out.println(r.get(i));
        }
        Collection newRoles = (Collection) roles.stream().map(role -> new SimpleGrantedAuthority("ROLE_" + role)).collect(Collectors.toList());
        return newRoles;
    }
